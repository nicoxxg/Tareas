...
createApp({
    ...
    methods:{
        cambiarPropietarioCurso(idNuevoProfesor){
            let idCurso = window.location.href.split('?=')[1]
            axios.put(`/api/curso/cambiar/profesor`,`idNuevoProfesor=${idNuevoProfesor}&idCurso=${idCurso}`).then((response) =>{
                window.location.href = "/Administrador/administrador.html"
            })

        },
        obtenerTodosProfesores(){
            axios.get("/api/profesor").then((response) =>{
        ...
        redireccionarDetalleAlumno(id){
            window.location.href = `/Administrador/detalleAlumno.html?=${id}` 
        },
